<?php
	...
	if($pos === 0)  
	{
		...
		while ($row = mysql_fetch_array($resultP)): 
			...
			$bodytag = "http://mon-paris.ru/catalog/".$row["LatCategoryName"]."/".$row["ProductIDStr"];
			header("Location: ".$bodytag,TRUE,301);
			exit();
		endwhile;
	}
	...
	{	
		...
		{			
			...
			if (array_key_exists($str, $vec_brand)) {
				...
				$strTitle = "Каталог модной женской одежды ".$brend." в Москве – MON-PARIS";
				$strDescription = "Купите стильные, качественные вещи из Европы от ".$brend." недорого в интернет-магазине MON-PARIS. Настоящее европейское качество в нашем городе! +7(495)-518-91-65";
			...
			if (array_key_exists($str, $vec_category)) {
				...
				$strTitle = "Модные женские ".$vec_category[$str][0]." купить в Москве от MON-PARIS";
	...
	if ($main === 1)
	{
		...
		$strDescription = "Вы ищете хорошую женскую одежду в Москве? Выберите стиль с Mon-Paris! В интернет-магазине или оффлайн, вы получите высочайшее качество у нас. Самые красивые и интересные вещи из Европы – спешите купить онлайн в Мон Париж.";
