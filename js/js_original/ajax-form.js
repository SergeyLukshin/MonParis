    ...
    script.ajaxform = {
        ...
        initHandler: function() {
            this.$forms.submit(function() {
				...
				//var $btn = $_this.find('.form_butt_2');
        ...
        go: function($form) {
            ...
            ajaxSettings.error = function(response) {
                window.alert("ajaxSettings.error");
            };
        ...
        validateByAjax: function($form, data) {
            ...
            if (data.status === 'ok') {
				...
				/*if (action == 'make_order') // выключаем кнопку, чтобы не было повторных отправлений заказа
				{
					var $formButtons = $form.find(':button,input[type="submit"]');
					$formButtons.attr("disabled", "disabled");
				}*/
                ...
                if (data.code === 'main') {
                    ...
                    if (action == 'message')
                    {
						$('#modal_feedback_1').scrollTop(0);
					}
					if (action == 'register')
                    {
						$('#modal_reg').scrollTop(0);
					}
					if (action == 'login')
                    {
						$('#modal_ent').scrollTop(0);
					}
